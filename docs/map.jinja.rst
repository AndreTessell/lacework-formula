...
We will see a quick setup to configure the ``lacework`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``lacework`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``lacework`` formulas under ``/srv/salt/lacework/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/lacework/parameters/dns:domain/
    ln -s dns:domain /srv/salt/lacework/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/lacework/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/lacework/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``lacework`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``lacework`` formulas under ``/srv/salt/lacework/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/lacework/parameters/roles
...
We create a configuration for the role ``lacework/server`` in ``/srv/salt/lacework/parameters/roles/lacework/server.yaml``:
...
We create another configuration for the role ``lacework/client`` in ``/srv/salt/lacework/parameters/roles/lacework/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles lacework/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles lacework/client
...
.. code-block:: console
    ...
    mv /srv/salt/lacework/parameters/dns:domain/  '/srv/salt/lacework/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-lacework-as-json:
      test.nop:
        - name: {{ lacework | json }}
...
Here is an example based on `template-formula/lacework/config/file.sls`_:
...
This ``sls`` file expose a ``lacework`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ lacework['winner'] }}
...
.. _template-formula/lacework/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/lacework/config/file.sls
